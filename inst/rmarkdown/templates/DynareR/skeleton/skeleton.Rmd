...
````` 
```{dynare DynareR1,eval=T,echo=T,comment=NULL,results='hide'} `r ''`
/*
...
stoch_simul(order=1, hp_filter=1600,format=pdf);
```  
`````


```{dynare BKK,eval=F,echo=F,comment=NULL}
/*
 * This file implements the multi-country RBC model with time to build,
 * described in Backus, Kehoe and Kydland (1992): "International Real Business
 * Cycles", Journal of Political Economy, 100(4), 745-775.
...
current directory named with their respective chunk name.


# Plotting the IRF

The Impulse Response Function (IRF) of the BKK model can be fetched using the following R chunk


```{r BKKFigure1,out.extra='trim={0cm 7cm 0cm 7cm},clip',fig.cap="Another of figure generated from Dynare software"}
