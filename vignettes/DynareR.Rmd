...
It is also assumed that `dynare` is installed in standard location.
...
```{r runAllDynare1}
run_models(model = 'DynareR/run_models*') # notice the * at the end
...
This function is a wrapper of `addpath` in `Octave`. If `dynare` is not installed in the standard location, use this function to add the `matlab` subdirectory.  By default, `DynareR` does this for if `dynare` is installed in the standard location. The default `path` is `/usr/lib/dynare/matlab` for `Linux`, `/usr/lib/dynare/matlab` for `macOS` and `c:/dynare/x.y/matlab` for `Windows`, where x.y is `Dynare` version number.
