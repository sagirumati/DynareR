...
Use this function to embed the graphs of Impulse Response Function (IRF) in
R Markdown document.
...
```
````
```{r irgg,fig.cap='Example of an IRF imported using include_IRF function'}
...
# Examples
  ...
  The [examples](https://github.com/sagirumati/DynareR/tree/master/inst/examples) folder contains R scripts for
demonstrating the use of DynareR. We use "example1" of the Dynare
example files for all the functions.
...
2. Please enusre that the "current working directory" is "examples" folder.
...
If you are using R Studio, you can open each of the files above and run the entire codes.
...
You can also open the `run_all.R` file in R Studio and run the entire code.

The outputs of each model (example1, bkk, agtrend etc) will be saved in a new folder with the model's name created in the current working directory (demo). The outputs of each DynareR function (write_mod, write_dyn, run_dynare) will be saved in a folder with the function's name.

# Working with existing mod files

The folder [DynareFiles](https://github.com/sagirumati/DynareR/tree/master/inst/examples/DynareFiles/) contains the Dynare built-in example files
...
3. Please ensure that the "current working directory" is `DynareFiles` folder.
...
please provide the folder,for example, as follows:
...
created in the current working directory (DynareFiles).
